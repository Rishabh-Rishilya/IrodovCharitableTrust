    ...
    <main>
      ...
      <section className="mx-auto max-w-5xl px-5 py-16 sm:px-8">
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-border">
          {Array.from({ length: 10 }, (_, i) => <div key={i} className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-primary">▣ Publication Document {i + 1}</p><p className="mt-1 text-sm text-[#3D4451]">202{Math.min(i, 6)} • {filters[(i % (filters.length - 1)) + 1]} • {(1.2 + i / 10).toFixed(1)} MB</p></div><button className="rounded-md bg-accent px-4 py-2 text-sm font-black text-white">Download</button></div>)}
        </div>
      </section>
    </main>
  );
