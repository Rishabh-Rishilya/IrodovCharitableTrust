  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        ...
        {(tab === "News" || tab === "Press Releases") && <div className="grid gap-5 md:grid-cols-3">{Array.from({ length: 6 }, (_, i) => <article key={i} className="overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-border"><Image src={placeholder(`${tab} ${i + 1}`, "400x240")} alt="" width={400} height={240} className="h-44 w-full object-cover" /><div className="p-5"><p className="text-xs font-bold text-muted">2026-06-{10 + i}</p><h2 className="mt-3 font-display text-xl font-black text-primary">{tab} Headline {i + 1}</h2><p className="mt-2 text-sm leading-6 text-muted">Two-line placeholder excerpt for the media update.</p></div></article>)}</div>}
