  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <p className="max-w-4xl text-lg leading-8 text-[#3D4451]">
        ...
        {[1, 2].map((item) => <div key={item} className="mt-12 grid gap-8 lg:grid-cols-2"><Image src={placeholder(`${title} Initiative ${item}`, "800x520")} alt="" width={800} height={520} className={`${item % 2 === 0 ? "lg:order-2" : ""} rounded-lg object-cover`} /><div className="self-center"><h2 className="font-display text-3xl font-black text-primary">Key Initiative {item}</h2><p className="mt-4 leading-7 text-[#3D4451]">Placeholder initiative details covering program design, local partners, beneficiary selection, implementation rhythm, and expected outcomes.</p></div></div>)}
