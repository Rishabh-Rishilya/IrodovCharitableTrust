    ...
    <main>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {services.map((service) => <div key={service} className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-primary/10"><span className="text-accent">◆</span> <span className="font-bold text-primary">{service}</span></div>)}
        </div>
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {[["2,500+", "Beneficiaries"], ["15", "Partner districts"], ["82%", "Follow-up completion"]].map(([num, label]) => <div key={label} className="rounded-lg bg-primary p-6 text-center text-white"><p className="font-display text-4xl font-black text-accent">{num}</p><p className="mt-2 text-sm">{label}</p></div>)}
        </div>
