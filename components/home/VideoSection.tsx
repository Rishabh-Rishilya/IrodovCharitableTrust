      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {videos.map(([title, duration, image]) => (
            <button key={title} type="button" onClick={() => setOpen(true)} className="group overflow-hidden rounded-lg bg-white text-left shadow-sm ring-1 ring-primary/10">
              <div className="relative">
                <Image src={`https://placehold.co/640x360/2D4B32/FFFFFF/png?text=${image}`} alt="" width={640} height={360} className="h-52 w-full object-cover" />
                <div className="absolute inset-0 bg-black/35" />
                <span className="absolute inset-0 m-auto grid size-16 place-items-center rounded-full bg-white text-2xl font-black text-primary transition group-hover:scale-105">▶</span>
        ...
        <Link href="/media" className="mt-8 inline-block font-bold text-accent">Watch More Videos →</Link>
      </div>
      {open && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-black/70 p-5" onClick={() => setOpen(false)}>
          <div className="w-full max-w-4xl" onClick={(event) => event.stopPropagation()}>
            ...
            <button type="button" onClick={() => setOpen(false)} className="mt-4 rounded-lg bg-white px-4 py-2 font-bold text-primary">Close</button>
          </div>
        </div>
      )}
    </section>
