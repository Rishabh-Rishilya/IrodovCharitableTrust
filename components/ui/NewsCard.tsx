...
export default function NewsCard({ date, title }: { date: string; title: string }) {
  return (
    <article className="overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-border">
      ...
      <div className="p-5">
        <p className="text-xs font-bold uppercase tracking-wide text-[#3D4451]">{date}</p>
        <h3 className="mt-3 font-display text-xl font-black">{title}</h3>
        <p className="mt-3 text-sm leading-6 text-[#3D4451]">Placeholder update from IRODOV programs, partnerships, and field impact.</p>
